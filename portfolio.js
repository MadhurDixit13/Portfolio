...
const observer1 = new IntersectionObserver(entries => {
    entries.forEach(entry => {
        const s = entry.target.querySelectorAll('.s');

        if (entry.isIntersecting) {
            for (let i = 0; i < 4; i++) {
                s[i].classList.remove('invisible');
                s[i].classList.add('s-animation');
            }
            return;
        }
        for (let i = 0; i < 4; i++) {
            s[i].classList.add('invisible');
            s[i].classList.remove('s-animation');
        }

    });
});

observer1.observe(document.querySelector('.s-wrapper'));
